...
> - §7 Phase 3 提到的"sleeptime 全自动蒸馏"**经评估已否决**，自动沉淀（auto + notify-after）是终态设计（见 ROADMAP H2）；
...
> 这是"自动沉淀"——agent 负责判断时机、提炼并直接写入，写完后告知用户；用户保留事后否决权（可让删/改）。曾设想的"sleeptime 后台自动压缩"经评估否决（理由见 §7），auto + notify-after 是终态设计。
...
- ~~**进阶：sleeptime 后台压缩**~~ → **经评估否决**。理由见 §7：LLM 自动判断"什么值得记"易产垃圾稀释检索（撞 H1），且有状态后台子系统与"CLI 解耦、被唤醒才活"哲学有张力。auto + notify-after 已足够。
...
- "记忆怎么更新" → §9.4：写 concept 必须带 frontmatter + 追加 log.md；自动沉淀（auto + notify-after）。
