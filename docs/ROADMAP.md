...
| H2 ✅ | 记忆"长"靠蒸馏,人会懒得记 | 提醒式半自动(纯文档):agent 主动判断时机+提炼,用户确认才写 | **半自动是终态设计,非待办**——全自动 sleeptime 经评估否决(LLM 产垃圾稀释检索,撞 H1;有状态后台子系统与"被唤醒才活"哲学冲突) |
...
- **为什么暂不动**:8 个工具 119 行,没到痛;拆要验证 FastMCP 跨模块注册机制,有不确定性;现在拆是过早优化。
...
| ~~4~~ | ~~H2~~ | ✅ 半自动蒸馏已落地(纯文档驱动) | 解决"懒得记",守住质量门禁;**全自动 sleeptime 已否决,半自动是终态设计** |
...
| ~~—~~ | ~~**GitHub 发布**~~ | ✅ 已推 github.com/sillogic/sill-ensoul,v0.1.0 tag 已打 | 首个公开版本上线 |
| — | **新 CLI 接入** | Claude/Codex 复制 (c) 薄壳 | 机械工作,需要时做 |
| — | PyPI 发布(可选) | `pip install sill-ensoul` 一行装 | 目前从 GitHub 装;发 PyPI 只加发版动作,不改代码,有需要再做 |

> **当前状态:核心闭环(唤醒→检索→引用→沉淀)已全部打通,已发布到 GitHub(v0.1.0)。** 项目已达设计终态——半自动蒸馏、编排者模式多 agent 协作都是定案,没有待办路线图。剩余的"新 CLI 接入""PyPI 发布"是按需的机械工作,非核心承诺缺口。
