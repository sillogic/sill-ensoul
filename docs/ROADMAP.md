...
| #2 ✅ | 记忆"长"靠蒸馏，人会懒得记 | 提醒式半自动（纯文档）：agent 主动判断时机 + 提炼，用户确认才写 | **半自动是终态设计，非待办**——全自动 sleeptime 经评估否决（LLM 产垃圾稀释检索，撞 #1；有状态后台子系统与"被唤醒才活"哲学冲突） |
...
| ~~5~~ | ~~#2~~ | ✅ 半自动蒸馏已落地（纯文档驱动） | 解决"懒得记"，守住质量门禁；**全自动 sleeptime 已否决，半自动是终态设计** |
