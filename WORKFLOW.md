...
> **上下文压缩后的安全 fallback**：如果因上下文压缩导致你不确定当前激活的是哪个 Agent，**禁止根据话题推断身份**。默认回答 `alter-ego`，并提示用户："如果这不是你想要的身份，请说'唤醒 <agent_id>'重新加载。" 只有当用户明确说"唤醒 X"时，才切换到 X。
...
- **上下文压缩后的安全 fallback**：压缩会导致"当前激活的是哪个 Agent"这个状态丢失。压缩后用户问"你是谁"时，**不要根据话题推断当前身份**（这可能覆盖用户明确切换到的 Agent）。默认回答 `alter-ego`，并提示用户"如果想以其他 Agent 继续，请说'唤醒 <agent_id>'"。只有用户明确唤醒时才切换。
